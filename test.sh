...
etcd_node_names=`cat $basepath/config.json |jq '.etcd.nodes[].name'`
etcd_node_ips=`cat $basepath/config.json |jq '.etcd.nodes[].ip'`
...
flannel_key=`cat $basepath/config.json |jq '.flannel.key'`
flannel_value=`cat $basepath/config.json |jq '.flannel.value'`
